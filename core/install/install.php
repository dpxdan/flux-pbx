...
//if the config file exists then disable the install page
	$config_exists = false;
	if (file_exists("/usr/local/etc/fluxpbx/config.conf")) {
		//bsd
		$config_exists = true;
	}
	elseif (file_exists("/etc/fluxpbx/config.conf")) {
		//linux
		$config_exists = true;
	}
	if ($config_exists) {
...
//if the config.php exists create the config.conf file
	if (!$config_exists) {
		if (file_exists("/usr/local/etc/fluxpbx/config.php")) {
			//bsd
			$config_path = "/usr/local/etc/fluxpbx";
		}
		elseif (file_exists("/etc/fluxpbx/config.php")) {
			//linux
			$config_path = "/etc/fluxpbx";
		}
...
//process and save the data
	if (count($_POST) > 0) {
		...
		if ($_REQUEST["step"] == "install") {
			...
			$output = shell_exec('cd '.$_SERVER["DOCUMENT_ROOT"].' && php /var/www/fluxpbx/core/upgrade/upgrade_schema.php');
			...
			//app defaults
			$output = shell_exec('cd '.$_SERVER["DOCUMENT_ROOT"].' && php /var/www/fluxpbx/core/upgrade/upgrade_domains.php');
...
//assign default values to the template
	...
	$view->assign("database_host", "127.0.0.1");
	$view->assign("database_port", "5432");
	$view->assign("database_name", "fluxpbx");
	$view->assign("database_username", "fluxpbx");
	$view->assign("database_password", "X2lzsHe3KJpgbVX5V7G5cXgbU0");
