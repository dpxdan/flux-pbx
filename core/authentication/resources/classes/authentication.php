			...
			foreach ($_SESSION['authentication']['methods'] as $name) {
				...
				if (file_exists($plugin)) {
					...
					if (!empty($array) && is_array($array)) {
						...
						$result['user_status'] = $array["user_status"];
			...
			if (!empty($_SESSION['authentication']['methods'])) {
				foreach ($_SESSION['authentication']['methods'] as $name) {
					if (!isset($_SESSION['authentication']['plugin'][$name]['authorized'])) {
						...
						$_SESSION['authentication']['plugin'][$name]['user_status'] = $_SESSION['user_status'];
		...
		//user is authorized - get user settings, check user cidr
			if ($authorized) {
				...
				//get the extensions that are assigned to this user
						...
						if (isset($_SESSION["user"]) && is_uuid($_SESSION["user_uuid"]) && is_uuid($_SESSION["domain_uuid"]) && !isset($_SESSION['user']['extension'])) {
								...
								if (is_array($result) && @sizeof($result) != 0) {
									foreach($result as $x => $row) {
										...
										$_SESSION['user_extension'] = $row["extension"];
										$_SESSION['user_extension_uuid'] = $row["extension_uuid"];
									}
								}
					...
					if (file_exists($_SERVER["PROJECT_ROOT"]."/app/flux_providers/app_config.php")) {
						if (isset($_SESSION["user"]) && is_uuid($_SESSION["user_uuid"]) && is_uuid($_SESSION["domain_uuid"]) && !isset($_SESSION['user']['providers'])) {
								//get the user providers list
								$_SESSION['user']['providers'] = null;
								$sql = "select ";
								$sql .= "p.flux_provider_uuid, ";
								$sql .= "p.flux_provider_name, ";
								$sql .= "p.flux_provider_api_url, ";
								$sql .= "p.flux_provider_auth_url, ";
								$sql .= "p.flux_provider_auth_type, ";
								$sql .= "p.flux_provider_auth_method, ";
								$sql .= "p.flux_provider_enabled, ";
								$sql .= "p.flux_provider_description, ";
								$sql .= "pu.domain_uuid, ";
								$sql .= "pu.user_uuid, ";
								$sql .= "pu.flux_provider_user_auth_token, ";
								$sql .= "pu.flux_provider_user_auth_username, ";
								$sql .= "pu.flux_provider_user_auth_token_renew, ";
								$sql .= "pu.flux_provider_user_auth_password ";
								$sql .= "from ";
								$sql .= "v_flux_provider_users as pu, ";
								$sql .= "v_flux_providers as p ";
								$sql .= "where ";
								$sql .= "pu.domain_uuid = :domain_uuid ";
								$sql .= "and p.flux_provider_uuid = pu.flux_provider_uuid ";
								$sql .= "and pu.user_uuid = :user_uuid ";
								$sql .= "and pu.flux_provider_user_enabled = 'true' ";
								$sql .= "order by ";
								$sql .= "p.flux_provider_name asc ";
								$parameters['domain_uuid'] = $_SESSION['domain_uuid'];
								$parameters['user_uuid'] = $_SESSION['user_uuid'];
								$database = new database;
								$result = $database->select($sql, $parameters, 'all');
								if (is_array($result) && @sizeof($result) != 0) {
									foreach($result as $x => $row) {
										//set the destination
									$token_renew = $row['flux_provider_user_auth_token_renew'];
										//build the user array
								$_SESSION['user']['providers'][$x]['api_username'] = $row['flux_provider_user_auth_username'];
								$_SESSION['user']['providers'][$x]['api_token'] = $row['flux_provider_user_auth_token'];
								$_SESSION['user']['providers'][$x]['api_token_new'] = $token_renew;
								$_SESSION['user']['providers'][$x]['provider_uuid'] = $row['flux_provider_uuid'];
								$_SESSION['user']['providers'][$x]['provider_name'] = $row['flux_provider_name'];
								$_SESSION['user']['providers'][$x]['api_url'] = $row['flux_provider_api_url'];
								$_SESSION['user']['providers'][$x]['api_url_auth'] = $row['flux_provider_auth_url'];
								$_SESSION['user']['providers'][$x]['api_auth_type'] = $row['flux_provider_auth_type'];
								$_SESSION['user']['providers'][$x]['api_auth_method'] = $row['flux_provider_auth_method'];
								$_SESSION['user']['providers'][$x]['api_description'] = $row['flux_provider_description'];
								$_SESSION['user']['providers'][$x]['api_provider_enabled'] = $row['flux_provider_enabled'];
...
$auth->domain_name = "sip.fluxpbx.com";
