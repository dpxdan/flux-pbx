...
//process the http post
	if (!empty($_POST) && @sizeof($_POST) > 0) {
		...
		//run source update
		if (!empty($action["upgrade_source"]) && permission_exists("upgrade_source") && !is_dir("/usr/share/examples/fluxpbx")) {
...
//show the content
	...
	if (permission_exists("upgrade_source") && !is_dir("/usr/share/examples/fluxpbx") && is_writeable($_SERVER["PROJECT_ROOT"]."/.git")) {
		...
		if ($branch_return_value == 0 && $commit_return_value == 0) {
			echo "	<a href='https://github.com/fluxpbx/fluxpbx/compare/".$git_current_commit."...".$git_current_branch."' target='_blank' title='".$git_current_commit."' onclick=\"event.stopPropagation();\"><i>".$git_current_branch."</i></a>";
		}
		echo "	</td>\n";
		echo "</tr>\n";
		echo "</table>\n";
