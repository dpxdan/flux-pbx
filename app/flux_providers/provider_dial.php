...
//includes files
	...
	require_once "resources/functions/provider_by.php";

//logging
	openlog("FluxPBX", LOG_PID | LOG_PERROR, LOG_LOCAL1);
...
//define PHP variables from the HTTP values
	...
	if ($request_method == 'POST') {
		$data = json_decode(file_get_contents("php://input"), TRUE);
		 $_SESSION['exten'] = $data['exten'];
		 $_SESSION['dest'] = $data['dest'];
//		var_dump($data);
print_r($_SESSION);
	}
...
//   $user_session = $_SESSION['session']['user_session_id'];
/*
//chec permissions
...
//get the http get values and set as php variables
	...
	$broadcast_domain = $_SESSION['domain_name'];
				...
				foreach ($broadcast_phone_number_array as $tmp_value) {
					...
					if (is_numeric($phone_1)) {
						...
						//prepare the string
							...
							$channel_variables .= ",origination_number=".$broadcast_caller_id_number;
							...
							$channel_variables .= ",outbound_caller_id_name='".$broadcast_caller_id_name."'";
							$channel_variables .= ",outbound_caller_id_number='".$broadcast_caller_id_number."'";
							...
							$channel_variables .= ",ignore_display_updates=true";
							$channel_variables .= ",movidesk_queue_id=6878";
							$channel_variables .= ",menu_in=true";
							$channel_variables .= ",agent_id=1022422";
							$channel_variables .= ",domain_uuid='".$_SESSION['domain_uuid']."'";
							$channel_variables .= ",domain='".$broadcast_domain."'";
...
//							$dialplan_variables = $dialplan->variables;
						//	$bridge_array[0] = $dialplan->bridges;
							...
							$bridge_array = outbound_route_to_bridge($_SESSION['domain_uuid'], $phone_1);
