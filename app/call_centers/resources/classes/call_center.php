...
//define the call center class
	if (!class_exists('call_center')) {
		class call_center {
			...
			public $queue_ivr_destination;
			public $queue_announce_position;
			...
			*/
			...
			public function dialplan() {
			    ...
			    if (strlen($this->queue_ivr_destination) > 0) {
				$dialplan["dialplan_details"][$y]["domain_uuid"] = $this->domain_uuid;
				$dialplan["dialplan_details"][$y]["dialplan_detail_tag"] = "action";
				$dialplan["dialplan_details"][$y]["dialplan_detail_type"] = "set";
				$dialplan["dialplan_details"][$y]["dialplan_detail_data"] = "queue_ivr_destination=" . $this->queue_ivr_destination;
				$dialplan["dialplan_details"][$y]["dialplan_detail_group"] = "2";
				$dialplan["dialplan_details"][$y]["dialplan_detail_order"] = $y * 10;
				$y++;
			}
			
	        if (strlen($this->queue_announce_position) > 0) {
								$dialplan["dialplan_details"][$y]["domain_uuid"] = $this->domain_uuid;
								$dialplan["dialplan_details"][$y]["dialplan_detail_tag"] = "action";
								$dialplan["dialplan_details"][$y]["dialplan_detail_type"] = "set";
								$dialplan["dialplan_details"][$y]["dialplan_detail_data"] = "result=\${luarun(callcenter-announce-position.lua \${uuid} " . $this->queue_name. '@'.$_SESSION["domain_name"]. '' .$this->queue_name.')}"';
								$dialplan["dialplan_details"][$y]["dialplan_detail_group"] = "2";
								$dialplan["dialplan_details"][$y]["dialplan_detail_order"] = $y * 10;
								$y++;
							}
