...
//get the messages waiting in the email queue
	while (true) {
		...
		$sql .= "and (message_type = 'sms' or message_type = 'mms' or message_type = 'chat') ";
		...
		if (is_array($message_queue) && @sizeof($message_queue) != 0) {
			foreach($message_queue as $row) {
				...
				if ($row['message_direction'] == 'inbound') {
					//$command = "/usr/bin/php /var/www/html/fluxpbx/app/messages/resources/service/message_send_inbound.php ";
				...
				if ($row['message_direction'] == 'outbound') {
					// /usr/bin/php /var/www/html/fluxpbx/app/messages/resources/service/message_send_outbound.php action=send&message_queue_uuid=&hostname=voip.fluxpbx.com
					//$command = "/usr/bin/php /var/www/html/fluxpbx/app/messages/resources/service/message_send_outbound.php ";
					$command .= "'message_queue_uuid=".$row['message_queue_uuid']."&hostname=".$row['hostname']."'";
				}
...
//how to use this feature
	// cd /var/www/html/fluxpbx && /usr/bin/php /var/www/html/fluxpbx/app/messages/resources/service/message_queue.php
