...
//process the user data and save it to the database
	if (count($_POST) > 0 && strlen($_POST["persistformvar"]) == 0) {
		...
		// handle media (if any)
			...
			$message_type = is_array($message_media) && @sizeof($message_media) != 0 ? 'mms' : 'chat';

		//get the contact uuid
			$sql = "select c.contact_uuid,u.user_uuid ";
			$sql .= "from v_contacts as c, v_users as u, v_contact_phones as p ";
			$sql .= "where u.contact_uuid = c.contact_uuid ";
			$sql .= "and p.contact_uuid = c.contact_uuid ";
			$sql .= "and p.phone_number like :phone_number ";
			$sql .= "and c.domain_uuid = :domain_uuid ";
			$parameters['phone_number'] = '%'.$phone_number.'%';
			$parameters['domain_uuid'] = $domain_uuid;
			$database = new database;
			$contact = $database->select($sql, $parameters, 'all');
		...
		//build the message array
			...
			$array['messages'][0]['contact_uuid'] = $row['contact_uuid'];
			$array['messages'][0]['message_uuid_to'] = $row['contact_uuid'];
			$array['messages'][0]['message_uuid_from'] = $_SESSION["contact_uuid"];
