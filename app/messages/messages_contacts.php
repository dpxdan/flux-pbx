...
//get the list
	...
	$sql = "select message_direction, message_from, message_to, contact_uuid, message_uuid_from,message_uuid_to ";
	$sql .= "from v_messages ";
...
//parse out numbers
	if (is_array($messages) && @sizeof($messages) != 0) {
		...
		foreach($messages as $message) {
			$number_from = preg_replace('{[\D]}', '', $message['message_to']);
			$number_to = preg_replace('{[\D]}', '', $message['message_from']);
			$user = $message['contact_uuid'];
			$user_contact = $_SESSION['user']['contact_uuid'];
			...
			switch ($message['message_direction']) {
				case 'inbound':
					if (!is_uuid($contact[$number_from]['contact_uuid'])) {
						$contact[$number_from]['contact_uuid'] = $message['message_uuid_from'];
					}
					break;
				case 'outbound':
					if (!is_uuid($contact[$number_to]['contact_uuid'])) {
						$contact[$number_to]['contact_uuid'] = $message['message_uuid_to'];
					}
					break;
			case null:
				if (!is_uuid($contact[$number_to]['contact_uuid'])) {
		...
		foreach($numbers as $number) {
			...
			//contact name/number
				if ($contact[$number]['contact_name_given'] != '' || $contact[$number]['contact_name_family'] != '') {
					...
					echo "	<span style='font-size: 80%; white-space: nowrap;'><a href=\"javascript:void(0)\" onclick=\"send_cmd('".PROJECT_PATH."/app/click_to_call/click_to_call.php?src_cid_name=".urlencode($number)."&src_cid_number=".urlencode($number)."&dest_cid_name=".urlencode($_SESSION['user']['extension'][0]['outbound_caller_id_name'])."&dest_cid_number=".urlencode($_SESSION['user']['extension'][0]['outbound_caller_id_number'])."&src=".urlencode($_SESSION['user']['extension'][0]['user'])."&dest=".urlencode($number)."&rec=true&ringback=pt-ring&auto_answer=true');\"><i class='fas fa-phone-alt' style='margin-right: 5px;'></i>".escape(format_phone($number)).'</a></span><br>';
					...
					if ($selected) {
						...
						$contact_html = (permission_exists('contact_view') ? "<a href='".PROJECT_PATH."/app/contacts/contact_edit.php?id=".$contact[$number]['contact_uuid']."' target='_blank'>".$contact_name."</a>" : $contact_name)." : <a href='#'>".escape(format_phone($number))."</a>";
