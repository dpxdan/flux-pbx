...
//css styles
	...
	//output messages
		if (is_array($messages) && @sizeof($messages) != 0) {
			...
			$from = $message['message_uuid_to'];
			$to = $message['message_uuid_from'];
			$user = $_SESSION['user']['user_uuid'];
			if ($from == $user) {
			$direction = "outbound";
			} else {
			$direction = "inbound";
				...
				//message bubble
					echo "<span class='message-bubble message-bubble-".($direction == 'inbound' ? 'me' : 'em')."'>";
						//contact image em
							if ($direction == 'inbound') {
								...
								foreach ($message_media[$message['message_uuid']] as $media) {
									if ($media['type'] != 'txt') {
										if ($media['type'] == 'jpg' || $media['type'] == 'jpeg' || $media['type'] == 'gif' || $media['type'] == 'png') {
											echo "<a href='#' onclick=\"display_media('".$media['uuid']."','".$media_source."');\" class='message-media-link-".($direction == 'inbound' ? 'em' : 'me')."'>";
										}
										else {
											echo "<a href='message_media.php?id=".$media['uuid']."&src=".$media_source."&action=download' class='message-media-link-".($direction == 'inbound' ? 'em' : 'me')."'>";
										}
