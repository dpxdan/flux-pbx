...
//define the process id file
	$pid_file = "/var/run/fluxpbx/email_send".".".$email_queue_uuid.".pid";
...
//make sure the /var/run/fluxpbx directory exists
	if (!file_exists('/var/run/fluxpbx')) {
		$result = mkdir('/var/run/fluxpbx', 0777, true);
		if (!$result) {
			die('Failed to create /var/run/fluxpbx');
		}
	}
...
//send the email
	if ($email_status) {
		...
		*/
		...
		/*
		$call_uuid = $headers["X-FluxPBX-Call-UUID"];
		if ($resend == true) {
			...
			if (!isset($email_log_uuid)) {
				//build insert array
					...
					$array['email_logs'][0]['domain_uuid'] = $headers["X-FluxPBX-Domain-UUID"];
					$array['email_logs'][0]['sent_date'] = 'now()';
					$array['email_logs'][0]['type'] = $headers["X-FluxPBX-Email-Type"];
