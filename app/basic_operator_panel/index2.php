	...
	if (isset($_REQUEST['status']) && $_REQUEST['status'] != '') {
	    ...
	    $custom_status = $_REQUEST['status'];
			switch ($user_status) {
				...
				$result['user_uuid'] = $_SESSION['user']['user_uuid'];
				$result['user_status'] = $user_status;
				$result['timestamp'] = 'now()';
				$result['domain_uuid'] = $_SESSION['domain_uuid'];
				$result['username'] = $_SESSION['username'];
				$result['insert_user'] = $_SESSION['user']['call_center_agent_uuid'];
				$result['update_user'] = $_SESSION['user']['user_uuid'];
		...
		//if call center app is installed then update the user_status
			if (is_dir($_SERVER["DOCUMENT_ROOT"].PROJECT_PATH.'/app/call_centers')) {
				//get the call center agent uuid
					$sql = "select call_center_agent_uuid from view_agents_xml ";
		        ...
		        $_SESSION['user']['call_center_agent_uuid'] = $call_center_agent_uuid;
						$fp = event_socket_create($_SESSION['event_socket_ip_address'], $_SESSION['event_socket_port'], $_SESSION['event_socket_password']);
						$switch_cmd .= "callcenter_config agent set status ".$call_center_agent_uuid." '".$user_status."'";
//			      $switch_cmd2 .= "callcenter_config agent set custom_status " . $call_center_agent_uuid . " '" . $user_status . "'";
						$switch_result = event_socket_request($fp, 'api '.$switch_cmd);
	//		      $switch_result2 = event_socket_request($fp, 'api ' . $switch_cmd2);
					}
		...
		//update the user state
					if (is_uuid($call_center_agent_uuid)) {
						$cmd = "api callcenter_config agent set custom_status ".$call_center_agent_uuid." '".$custom_status."'";
					...
					$cmd = "api callcenter_config agent set state ".$call_center_agent_uuid." 'Idle'";
					$response = event_socket_request($fp, $cmd);
				...
				//update do not disturb
					...
					$sql = "select profile_name from view_agents ";
					$sql .= "where domain_uuid = :domain_uuid ";
					$sql .= "and user_uuid = :user_uuid ";
				...
				//delete extension from the cache
					$cache = new cache;
					$cache->delete("directory:".$extension."@".$_SESSION['user']['domain_name']);
					if(strlen($number_alias) > 0){
...
if (is_uuid($call_center_agent_uuid)) {
	...
	$fp = event_socket_create($_SESSION['event_socket_ip_address'], $_SESSION['event_socket_port'], $_SESSION['event_socket_password']);
	$switch_cmd .= "callcenter_config agent set state ".$call_center_agent_uuid." '".$state."'";
	$switch_result = event_socket_request($fp, 'api '.$switch_cmd);
}
exit;
}
...
$refresh_default = 2500; //milliseconds
...
//ajax refresh
	...
	var source_url = 'resources/content2.php?' <?php if (isset($_GET['debug'])) { echo " + '&debug'"; } ?>;
...
//refresh controls
	...
	function refresh_stop() {
		clearInterval(interval_timer_id);
		if (document.getElementById('refresh_state')) { document.getElementById('refresh_state').innerHTML = "<?php echo button::create(['type'=>'button','title'=>$text['label-refresh_pause'],'icon'=>'pause','onclick'=>'refresh_start()']); ?>"; }
	}
...
//park call
	function park_call(uuid) {
		if (uuid != '') {
			cmd = get_park_cmd(uuid);
			if (cmd != '') {
				send_cmd(cmd);
			}
		}
...
//hide/show destination input field
	...
	function get_originate_cmd_new(source, destination) {
		url = "/app/click_to_call/click_to_call.php?rec=true&auto_answer=false&dest_cid_name=Callcenter&dest_cid_number="+destination+"&src=" + source + "&src_cid_number="+source+"&src_cid_name="+source+"&dest=" + destination
		return url;
	}
	...
	function get_park_cmd(uuid) {
		url = "exec.php?cmd=uuid_park&uuid=" + uuid;
		return url;
	}
