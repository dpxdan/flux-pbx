...
if (!class_exists('basic_operator_panel')) {
	class basic_operator_panel {
		...
		public function __construct() {
			...
			if (!isset($this->call_group)) {
				$this->call_group = user_extension_group();
			}
			if (!isset($this->user_group_level)) {
				$this->user_group_level = user_group_level();
			}
			if (!isset($this->user_uuid)) {
				$this->user_uuid = $_SESSION['user_uuid'];
			}
			if (!isset($this->extension_uuid)) {
				$this->extension_uuid = user_extension_uuid();
			}		
		}
		...
		public function call_activity() {
			...
			//get the extensions and their user status
				...
				$sql .= "left outer join v_user_groups as g on ( u.user_uuid = g.user_uuid and g.domain_uuid = :domain_uuid ) ";
				$sql .= "left outer join view_agents as va on ( va.user_uuid = g.user_uuid and g.domain_uuid = :domain_uuid ) ";
				$sql .= "where ";
				$sql .= "e.enabled = 'true' and ";
				if ($this->call_group == 'admins') {
				
				}
				else if (if_group('superadmin')) {
                //
				}  
				else {
				$sql .= "e.call_group = :call_group and ";
				...
				if ($this->call_group == 'admin') {
				//$parameters['call_group'] = $_SESSION['call_group'];
				}
				else if (if_group('superadmin')) {
				//$parameters['call_group'] = $_SESSION['call_group'];
				} 
				else {
				$parameters['call_group'] = $this->call_group;
				...
				if (isset($extensions)) {
					foreach($extensions as &$row) {
						if ($row['user_uuid'] != '') {
							...
							$ext_agent_status[$row['user_uuid']] = $row['agent_status'];
							$ext_reg_user[$row['user_uuid']] = $row['reg_user'];
							$ext_profile_user[$row['user_uuid']] = $row['profile_name'];
							unset($row['agent_status']);
							unset($row['user_status']);
							unset($row['reg_user']);
							unset($row['profile_name']);
						}
					}
				}
					...
					foreach($extensions as &$row) {
						...
						//normalize the array
							if ($found) {
								...
								$array[$x]["profile_name"] = $field['profile_name'];
		...
		public function get_contact() {
					...
					$sql .= "view_contacts ";
					$sql .= "where ";
					$sql .= "user_uuid = :user_uuid and ";
					$sql .= "domain_uuid = :domain_uuid ";
					$sql .= "order by ";
					$sql .= "contact_uuid asc limit 1 ";
					$parameters['domain_uuid'] = $_SESSION['domain_uuid'];
					$parameters['user_uuid'] = $user_uuid;
					$database = new database;
					$contact_uuid = $database->select($sql, $parameters);
		
					//store extension status by user uuid
					if (isset($contact_uuid)) {
						foreach ($contact_uuid as &$row) {
							if ($row['contact_uuid'] != '') {
								$contact_uuid_user[$row['contact_uuid']] = $row['contact_uuid'];
								return $contact_uuid_user;
								//unset($row['contact_uuid']);
							}
						}
					}
...
public function get_register(){
				...
				global $extension_user_profile;
					...
					//get the extensions and their user status
						...
						$sql .= "domain_uuid = :domain_uuid ";
						$sql .= "order by ";
						$sql .= "profile_name asc limit 1 ";
						$parameters['domain_uuid'] = $_SESSION['domain_uuid'];
						$parameters['user_uuid'] = $extension_user_profile;
						$database = new database;
						$extension_uuid_profile = $database->select($sql, $parameters);
		
					//store extension status by user uuid
						if (isset($extension_uuid_profile)) {
							foreach($extension_uuid_profile as &$row) {
								if ($row['profile_name'] != '') {
									$profile_name[$row['profile_name']] = $row['profile_name'];
									return $profile_name;
									unset($row['profile_name']);
								}
							}
...
public function get_park(){
							...
							//define the global variable
				global $extension_park;
		
					//get the extensions and their user status
						$sql = "select ";
						$sql .= "extension,effective_caller_id_number ";
						$sql .= "from ";
						$sql .= "view_api_phone ";
						$sql .= "where ";
						$sql .= "extension = :extension_park and ";
						$sql .= "domain_uuid = :domain_uuid ";
						$sql .= "order by ";
						$sql .= "extension asc limit 1 ";
						$parameters['domain_uuid'] = $_SESSION['domain_uuid'];
						$parameters['extension_park'] = $extension_park;
						$database = new database;
						$extension_park = $database->select($sql, $parameters);
	
						
				if (isset($extension_park)) {
									foreach($extension_park as &$row) {
										if ($row['extension'] != '') {
											$ext_park_extension[$row['extension']] = $row['extension'];
											$ext_park_number[$row['effective_caller_id_number']] = $row['effective_caller_id_number'];
											unset($row['extension']);
											unset($row['effective_caller_id_number']);
										}
									}
								}
