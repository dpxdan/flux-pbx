...
//add the search string
	...
	if (strlen($search) > 0) {
		...
		$sql_search .= "	or lower(api_redirect) like :search ";
...
//show the content
	...
	if (permission_exists('restapi_add')) {
		...
		echo button::create(['type'=>'button','label'=>$text['button-restapi_endpoint'],'icon'=>'th','link'=>'rest_api_endpoints.php']);
	}
	...
	echo th_order_by('api_redirect', $text['label-restapi_redirect'], $order_by, $order);
	...
	if (is_array($rest_api) && @sizeof($rest_api) != 0) {
		...
		foreach ($rest_api as $row) {
			...
			echo "	<td>".escape($row['api_redirect'])."</td>\n";
