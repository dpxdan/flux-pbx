...
--set default variables
	...
	if (session:ready()) then
		...
		if (not default_language) then default_language = 'pt'; end
		if (not default_dialect) then default_dialect = 'BR'; end
		if (not default_voice) then default_voice = 'karina'; end
	end
	...
	local sql = "SELECT v_call_center_agents.*,v_users.user_status,v_extension_users.extension_uuid FROM v_call_center_agents, v_users, v_extension_users ";
	sql = sql .. "WHERE v_call_center_agents.domain_uuid = :domain_uuid AND v_call_center_agents.user_uuid = v_users.user_uuid AND v_extension_users.user_uuid = v_call_center_agents.user_uuid ";
	...
	if (agent_authorized == 'true') then
		...
		cmd = "sched_api +5 none callcenter_config agent set custom_status "..agent_uuid.." '"..user_status.."'";
		freeswitch.consoleLog("notice", "[user custom status][login] "..cmd.."\n");
		result = api:executeString(cmd);


		cmd = "sched_api +5 none callcenter_config agent set provider_status "..agent_uuid.." '"..extension_uuid.."'";
		freeswitch.consoleLog("notice", "[user custom status][login] "..cmd.."\n");
		result = api:executeString(cmd);
...
--			session:execute("playback", sounds_dir.."/ivr/ivr-thank_you.wav");
	...
	--set the variables
		if (session ~= nil and session:ready()) then
			session:setVariable("agent_uuid", agent_uuid);
			session:setVariable("agent_name", agent_name);
			session:setVariable("agent_id", agent_id);
			session:setVariable("custom_status", user_status);
			session:setVariable("user_uuid", user_uuid);
			session:setVariable("provider_status", extension_uuid);
