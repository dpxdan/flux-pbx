...
function config()
	if (file_exists("/usr/local/etc/fluxpbx/config.conf")) then
		return "/usr/local/etc/fluxpbx/config.conf";
	elseif (file_exists("/etc/fluxpbx/config.conf")) then
		return "/etc/fluxpbx/config.conf";
	else
...
function load_config()
	...
	if (event_socket_password == nil) then
		event_socket_password = 'Flux223304442Dev2516';
	end
