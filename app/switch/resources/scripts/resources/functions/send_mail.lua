...
if (email_queue_enabled == 'true') then
	function send_mail(headers, email_from, email_address, email_message, email_file)
		...
		local domain_uuid = headers["X-FluxPBX-Domain-UUID"];
		local domain_name = headers["X-FluxPBX-Domain-Name"];
		local email_type = headers["X-FluxPBX-Email-Type"] or 'info';
		local call_uuid = headers["X-FluxPBX-Call-UUID"];
		local local_after_email = headers["X-FluxPBX-local_after_email"] or '';
	...
	--local headers = {
	--	["X-FluxPBX-Domain-UUID"] = '2d171c4c-b237-49ca-9d76-9cffc1618fa7';
	--	["X-FluxPBX-Domain-Name"] = 'domain.com';
	--	["X-FluxPBX-Email-Type"]	= 'voicemail';
	--}
	...
	if freeswitch then
		function send_mail(headers, from, address, message, file)
			...
			local domain_name = headers["X-FluxPBX-Domain-Name"]
			local email_type = headers["X-FluxPBX-Email-Type"] or 'info'
			local call_uuid = headers["X-FluxPBX-Email-Call-UUID"]
			local db = dbh or Database.new('system')
			local settings = Settings.new(db, domain_name, domain_uuid)
			local xheaders = "{"
