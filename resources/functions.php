...
//get accountcode
	...
	if (!function_exists('user_extension_uuid')) {
		function user_extension_uuid($domain_name = null) {
			global $domain_uuid;
			$sql = "select e.* ";
	...
	if (!function_exists('user_agent_uuid')) {
		function user_agent_uuid($domain_name = null) {
			global $domain_uuid;
			$sql = "select c.call_center_agent_uuid ";
			$sql .= "from v_call_center_agents as c, ";
			$sql .= "v_users as u ";
			$sql .= "where c.user_uuid = u.user_uuid ";
			$sql .= "and u.user_uuid = :user_uuid ";
			$sql .= "and c.domain_uuid = :domain_uuid ";
			$sql .= "and u.enabled = 'true' ";
			$sql .= "order by c.agent_id asc limit 1 ";
			$parameters['user_uuid'] = $_SESSION['user']['user_uuid'];
			$parameters['domain_uuid'] = $_SESSION['domain_uuid'];
			$database = new database;
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_agent_uuid = $row['call_center_agent_uuid'];
			}
	
			return $user_agent_uuid;
		}
	}
	
	if (!function_exists('user_extension_status')) {
		function user_extension_status($extension_uuid, $domain_name = null) {
			global $domain_uuid;
			$sql = "select e.*,eu.status ";
			$sql .= "from v_extensions as e, ";
			$sql .= "sip_registrations as eu ";
			$sql .= "where e.extension = eu.sip_user ";
			$sql .= "and e.extension_uuid = :extension_uuid ";
			$sql .= "and e.domain_uuid = :domain_uuid ";
			$sql .= "and e.user_context = eu.sip_host ";
			$sql .= "order by e.extension asc limit 1 ";
			...
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_extension_status = $row['status'];
			}
	
			return $user_extension_status;
		}
	}
	
	if (!function_exists('user_extension_profile')) {
		function user_extension_profile($extension_uuid, $domain_name = null) {
			global $domain_uuid;
			$sql = "select eu.profile_name ";
			$sql .= "from v_extensions as e, ";
			$sql .= "sip_registrations as eu ";
			$sql .= "where e.extension = eu.sip_user ";
			$sql .= "and e.extension_uuid = :extension_uuid ";
			...
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_extension_profile = $row['profile_name'];
			}
	
			return $user_extension_profile;
		}
	}
	
	if (!function_exists('user_provider_domain')) {
		function user_provider_domain($provider_name) {
			$sql = "select d.domain_name ";
			$sql .= "from v_domains as d, ";
			$sql .= "v_iungo_providers as p ";
			$sql .= "where d.domain_uuid = p.domain_uuid ";
			$sql .= "and p.iungo_provider_name = :provider_name ";
			$sql .= "order by p.iungo_provider_name asc limit 1 ";
			$parameters['provider_name'] = $provider_name;
			$database = new database;
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_provider_domain = $row['domain_name'];
			}
	
			return $user_provider_domain;
		}
	}
	
	if (!function_exists('user_provider_domain_uuid')) {
		function user_provider_domain_uuid($provider_name) {
			...
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_provider_domain_uuid = $row['domain_uuid'];
			}
	
			return $user_provider_domain_uuid;
		}
	}
	
	if (!function_exists('user_extension_group')) {
		function user_extension_group($domain_name = null) {
			global $domain_uuid;
			$sql = "select e.* ";
			$sql .= "from v_extensions as e, ";
			$sql .= "v_extension_users as eu ";
	...
	if (!function_exists('user_group_level')) {
		function user_group_level($domain_name = null) {
			global $domain_uuid;
			$sql = "select g.* ";
			$sql .= "from view_users as g ";
			$sql .= "where g.user_uuid = :user_uuid ";
			$sql .= "and g.domain_uuid = :domain_uuid ";
			$sql .= "and g.user_enabled = 'true' ";
			$sql .= "order by g.username asc limit 1 ";
			$parameters['user_uuid'] = $_SESSION['user']['user_uuid'];
			...
			$row = $database->select($sql, $parameters, 'row');
			if (is_array($row)) {
				$user_group_level = $row['group_level'];
			}
	...
	if (!function_exists('user_extension_exists')) {
		function user_extension_exists($domain_name = null) {
			global $domain_uuid;
			$sql = "select e.* ";
			$sql .= "from v_extensions as e, ";
			$sql .= "v_extension_users as eu ";
