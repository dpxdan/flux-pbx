...
//create token
	...
	$extension_json_token = $object->create('/app/flux_phone/extensions_json.php');
...
//set template variables
	...
	//domain json token
		...
		$view->assign('extension_json_token_name', $extension_json_token['name']);
		$view->assign('extension_json_token_hash', $extension_json_token['hash']);
	...
	//theme settings
		if (is_array($_SESSION['theme']) && @sizeof($_SESSION['theme']) != 0) {
			...
			//pre-process some settings
				...
				$settings_array['theme']['footer'] = isset($settings_array['theme']['footer']) ? $settings_array['theme']['footer'] : '&copy; '.$text['theme-label-copyright'].' 2015 - '.date('Y')." <a href='https://flux.net.br' class='footer' target='_blank'>Flux Telecom</a> ".$text['theme-label-all_rights_reserved'];
	...
	//domain selector control
		...
		$extension_selector_enabled = permission_exists('extension_view') ? true : false;
		$view->assign('extension_selector_enabled', $extension_selector_enabled);
	...
	//login state
		...
		$registered = isset($_SESSION['user_extension_uuid']) && $_SESSION['user_extension_uuid'] != '' ? true : false;
		$view->assign('registered', $registered);
		
		//phone application path
		$view->assign('extension_app_path', PROJECT_PATH.(file_exists($_SERVER['DOCUMENT_ROOT'].'/app/flux_phone/flux.php') ? '/app/flux_phone/flux.php' : '/core/flux_phone/flux.php'));
