...
function outbound_api_to_bridge($domain_uuid, $destination_number, array $channel_variables=null) {
	...
	$sql .= "and (hostname = :hostname or hostname is null) ";
	$sql .= "and d.app_uuid != 'c03b422e-13a8-bd1b-e42b-b6b9b4d27ce4' ";
	$sql .= "and d.dialplan_enabled = 'true' ";
	$sql .= "order by d.domain_uuid,  d.dialplan_order, dd.dialplan_detail_order ";
	$parameters['hostname'] = $hostname;
	$database = new database;
	...
	function save_call_center_xml() {
		...
		if (!empty($_SESSION['switch']['call_center']['dir'])) {
			...
			if (!empty($call_center_queues)) {
					...
					if (!empty($result)) {
						foreach ($result as &$row) {
							//get the values from the db and set as php variables
								...
								$agent_call_group = $row["call_group"];
								$agent_api_key = $row["api_key"];
								...
								$provider_status = $row["user_uuid"];
								$extension_uuid = $row["ext_uuid"];
								...
								$tmp_confirm = "execute_on_pre_bridge='lua confirm.lua',confirm=false,leg_timeout=".$agent_call_timeout;
							...
							$v_agents .= "agent_uuid=\"$agent_uuid\" ";
							$v_agents .= "contact=\"$tmp_agent_contact\" ";
							$v_agents .= "status=\"$agent_status\" ";
							$v_agents .= "telephony_status=\"$telephony_status\" ";
							$v_agents .= "custom_status=\"$custom_status\" ";
							$v_agents .= "provider_status=\"$provider_status\" ";
							$v_agents .= "extension_uuid=\"$extension_uuid\" ";
				...
				//set the path
					if (file_exists('/usr/share/examples/fluxpbx/resources/templates/conf')) {
						$path = "/usr/share/examples/fluxpbx/resources/templates/conf";
					}
					else {
 ...
 * to connect to the running fluxpbx process and close the port internally using debug symbols. This
 * function requires fluxpbx to be compiled with the --enable-debug flag.
 * @param string $port
 * @return void
 */
function force_close_port(string $port): void {
	...
	//get the pid of fluxpbx
	$pid = exec('pidof fluxpbx');
	...
	//get a list of the current connections owned by fluxpbx
	$connections = "";
