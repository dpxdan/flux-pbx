...
/**
 * Subscribes to the event socket of the FluxPBX (c) Event Socket Server
...
class event_socket {
	...
	/**
	 * Connect to the FluxPBX (c) event socket server
	 ...
	 * @global array $conf Global configuration used in fluxpbx/config.conf
	 * @param string $host Host or IP address of FluxPBX event socket server. Defaults to 127.0.0.1
	 * @param string $port Port number of FluxPBX event socket server. Defaults to 8021
	 * @param string $password Password of FluxPBX event socket server. Defaults to Flux223304442Dev2516
	...
	public function connect($host = null, $port = null, $password = null, $timeout_microseconds = 30000) {
		...
		$password = $password ?? $conf['switch.event_socket.password'] ?? $conf['event_socket.password'] ?? 'Flux223304442Dev2516';
	...
	/**
	 * Tests if connected to the FluxPBX Event Socket Server
	...
	/**
	 * Send a command to the FluxPBX Event Socket Server
	 * <p>Multi-line commands can be sent when separated by '\n'</p>
	 * @param string $cmd Command to send through the socket
	 * @return mixed Returns the response from FluxPBX or false if not connected
	 * @depends read_event()
	 */
	 ...
	 * @param resource|bool $fp Sets the current FluxPBX resource
	...
	/**
	 * Create uses a singleton design to return a connected socket to the FluxPBX Event Socket Layer
	...
	/**
	 * Sends an API command to FluxPBX using asynchronous (non-blocking) mode
