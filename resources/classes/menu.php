			...
			//begin navbar code
				...
				if ($menu_brand) {
					//define menu brand mark
						$menu_brand_text = (!empty($_SESSION['theme']['menu_brand_text']['text'])) ? escape($_SESSION['theme']['menu_brand_text']['text']) : "FluxPBX";
				...
				//current user
					...
					//current extension		
					if (isset($_SESSION['user_extension_uuid']) && $_SESSION['user_extension_uuid'] != '' && isset($_SESSION['user_extension']) && $_SESSION['user_extension'] != '' && $_SESSION['theme']['phone_icon_visible']['text'] == "true") {
	
	
	$html .= "		<li class='nav-item'>\n";
	$html .= "			<a class='header_phone' id='launchPhoneFlux' href='#' title=\"".$this->text['theme-label-fluxphone']."\"><i class='fas fa-".($_SESSION['theme']['body_header_icon_phone']['text'] != '' ? $_SESSION['theme']['body_header_icon_phone']['text'] : 'phone')." fa-lg fa-fw' style='margin-top: 6px; margin-right: 5px;'></i>".$_SESSION['user_extension']."</a>";
	$html .= "		</li>\n";
						$html .= "  <script>\n";
						$html .= "  var phoneurl      = '".PROJECT_PATH."/app/flux_phone/flux.php?id=" . escape($_SESSION['user_extension_uuid']) . "',\n";
						$html .= "      features = 'name=fluxPhone,width=365,height=580'\n";
						$html .= "      $('#launchPhoneFlux').on('click', function(event) { \n";
						$html .= "          event.preventDefault() \n";
						// This is set when the phone is open and removed on close
						$html .= "          if (!localStorage.getItem('fluxPhone')) { \n";
						$html .= " $('#phoneDiv').removeClass('splash_show');\n";
						$html .= " $('#phoneDiv').addClass('splash_hide');\n";
						$html .= "              window.open(phoneurl, 'fluxPhone', features)\n";
						$html .= "              return false\n";
						$html .= "          } else { \n";
						$html .= " $('#phoneDiv').removeClass('splash_show');\n";
						$html .= " $('#phoneDiv').addClass('splash_hide');\n";
						$html .= "              window.alert('Seu ramal já está em uso.')\n";
						$html .= "          }\n";
						$html .= "      })\n";
						$html .= "	function updatevariable(data) { \n";
						$html .= "		value = data;\n";
						$html .= "  } \n";
						$html .= "  </script>\n";
					}
...
// 				}
			...
			//header: left
				...
				$body_header_brand_text = !empty($_SESSION['theme']['body_header_brand_text']['text']) ? escape($_SESSION['theme']['body_header_brand_text']['text']) : "FluxPBX";
			...
			//current extension		
			if (isset($_SESSION['user_extension_uuid']) && $_SESSION['user_extension_uuid'] != '' && isset($_SESSION['user_extension']) && $_SESSION['user_extension'] != '' && $_SESSION['theme']['phone_icon_visible']['text'] == "true") {
			$html .= "<span id='phoneDiv' class='splash_show' style='display: inline-block; padding-right: 30px; font-size: 90%;'>\n";
			$html .= "<a id='launchPhoneFlux' href='#' title=\"".$this->text['theme-label-fluxphone']."\"><i class='fas fa-".($_SESSION['theme']['body_header_icon_phone']['text'] != '' ? $_SESSION['theme']['body_header_icon_phone']['text'] : 'phone')." fa-lg fa-fw' style='margin-top: 6px; margin-right: 5px;'></i>".$_SESSION['user_extension']."</a>";
				$html .= "</span>\n";
				$html .= "  <script>\n";
				$html .= "  var phoneurl      = '".PROJECT_PATH."/app/flux_phone/flux.php?id=" . escape($_SESSION['user_extension_uuid']) . "',\n";
				$html .= "      features = 'name=fluxPhone,width=365,height=650'\n";
				$html .= "      $('#launchPhoneFlux').on('click', function(event) { \n";
				$html .= "          event.preventDefault() \n";
				// This is set when the phone is open and removed on close
				$html .= "          if (!localStorage.getItem('fluxPhone')) { \n";
