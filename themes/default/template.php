...
{*//external css files *}
<link rel='stylesheet' type='text/css' href='{$project_path}/resources/bootstrap/css/now-ui-dashboard.css.php'>
	...
	<link rel='stylesheet' type='text/css' href='{$project_path}/resources/fontawesome6/css/all.min.css.php'>
...
{*//remote javascript *}
	...
	<script language='JavaScript' type='text/javascript' src='{$project_path}/resources/bootstrap/js/bootstrap-notify.js.php'></script>
	
	<script language='JavaScript' type='text/javascript'>{literal}window.FontAwesomeConfig = { autoReplaceSvg: false }{/literal}</script>
	<script language='JavaScript' type='text/javascript' src='{$project_path}/resources/fontawesome6/js/solid.min.js.php' defer></script>
	<script language='JavaScript' type='text/javascript' src='{$project_path}/resources/javascript/jsrsasign/jsrsasign-all-min.js.php'></script>
...
{*//local javascript *}
		...
		//message bar display
			...
			function display_message(msg, mood, delay) {
				...
				if (msg !== '') {
					...
					//showNotification('top','left',msg);
				}
				}
			
			function showNotification(message, mood, align) {
               mood = mood !== undefined ? mood : 'primary';
               align = align !== undefined ? align : 'top';
               if (mood === 'positive') {
               mood = 'success';
               } else {
               mood = 'danger';
	...
	$(document).ready(function() {
		...
		//domain selector controls
			...
			//extension selector controls
				{if $extension_selector_enabled}
					{literal}
					$('.extension_selector_extension').on('click', function() { show_extensions(); });
					$('#header_extension_selector_extension').on('click', function() { show_extensions(); });
					$('#extension_hide').on('click', function() { hide_extensions(); });
			
					function show_extensions() {
						search_extensions('extension_list');
			
						$('#extension_visible').val(1);
						var scrollbar_width = (window.innerWidth - $(window).width()); //gold: only solution that worked with body { overflow:auto } (add -ms-overflow-style: scrollbar; to <body> style for ie 10+)
						if (scrollbar_width > 0) {
							$('body').css({'margin-right':scrollbar_width, 'overflow':'hidden'}); //disable body scroll bars
							$('.navbar').css('margin-right',scrollbar_width); //adjust navbar margin to compensate
							$('#extension_container').css('right',-scrollbar_width); //extension container right position to compensate
						}
						$(document).scrollTop(0);
						$('#extension_container').show();
						$('#extension_block').animate({marginRight: '+=300'}, 400, function() {
							$('#extension_search').trigger('focus');
						});
					}
			
					function hide_extensions() {
						$('#extension_visible').val(0);
						$(document).ready(function() {
							$('#extension_block').animate({marginRight: '-=300'}, 400, function() {
								$('#extension_search').val('');
								$('.navbar').css('margin-right','0'); //restore navbar margin
								$('#extension_container').css('right','0'); //extension container right position
		...
		//initialize bootstrap tempusdominus (calendar/datetime picker) plugin
			...
			$(function() {
				...
				//define formatting of individual classes
					...
					$('.datetimehourpicker').datetimepicker({ format: 'HH:mm', });
					$('.datetime_extension_picker_from').datetimepicker({ format: 'HH:mm', });
					$('.datetime_extension_picker_to').datetimepicker({ format: 'HH:mm', });
	...
	//misc functions
		...
		function open_extension(extension_uuid) {
						 var phoneurl      = '/app/flux_phone/flux.php?id='+extension_uuid+'',
						     features = 'name=fluxPhone,width=365,height=650'
						  $('#launchPhoneFlux').on('click', function(event) {
						         event.preventDefault()
						if (!localStorage.getItem('fluxPhone')) {
						$('#phoneDiv').removeClass('splash_show');
						$('#phoneDiv').addClass('splash_hide');
						window.open(phoneurl, 'fluxPhone', features)
						return false
						         } 
						else {
						$('#phoneDiv').removeClass('splash_show');
	...
	{*//extension selector *}
	function search_extensions(element_id) {
	...
	xhttp.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			//update the domain count
			document.getElementById('extension_count').innerText = '('+ obj.length +')';
	
			//add new options from the json results
			for (var i=0; i < obj.length; i++) {
				
				//get the variables
				extension_uuid = obj[i].extension_uuid;
				extension = obj[i].extension;
				if (obj[i].effective_caller_id_name != null) {
				//	domain_description = DOMPurify.sanitize(obj[i].domain_description);
				}
				...
				//add a div title
				div.title = obj[i].extension;
	
				//add a css class
				div.classList.add("extension_list_item");
				...
				//set the active domain style 
				if ('{$extension_uuid}' == obj[i].extension_uuid) {
					div.style.background = '{$domain_active_background_color}';
					div.style.fontWeight = 'bold';
					//div.classList.add("domains_list_item_active");
					//var item_description_class = 'domain_active_list_item_description';
				...
				//set link on domain div in list
//				div.setAttribute('onclick',"open_extension(" + obj[i].extension_uuid +")");
				div.setAttribute('onclick',"window.location.href='{$extension_app_path}?id=" + obj[i].extension_uuid + "&extension_change=true';");
	
				//define domain link text and description (if any)
				link_label = obj[i].extension;
				if (obj[i].effective_caller_id_name != null) {
					link_label += " <span class='extension_list_item_number_alias' title=\"" + obj[i].effective_caller_id_name + "\">" + obj[i].effective_caller_id_name + "</span>";
				}
				var a_tag = document.createElement('a');
				a_tag.setAttribute('href','manage:'+obj[i].extension);
				a_tag.setAttribute('onclick','event.preventDefault();');
				a_tag.innerHTML = link_label;
				div.appendChild(a_tag);
	...
	search = document.getElementById('extension_search');
	if (search.value) {
		//xhttp.open("GET", "/core/domains/domain_list.php?search="+search.value, true);
		xhttp.open("GET", "/app/flux_phone/extensions_json.php?search="+search.value+"&{$extension_json_token_name}={$extension_json_token_hash}", true);
	}
	else {
		//xhttp.open("GET", "/core/domains/domain_list.php", true);
		xhttp.open("GET", "/app/flux_phone/extensions_json.php?{$extension_json_token_name}={$extension_json_token_hash}", true);
	}
	xhttp.send();
...
<body>
	...
	{*//domain selector *}
		...
		{*//extension selector *}
			{if $registered && $extension_selector_enabled}
		
				<div id='extension_container'>
					<input type='hidden' id='extension_visible' value='0'>
					<div id='extension_block'>
						<div id='extension_header'>
							<input id='extension_hide' type='button' class='btn' style='float: right' value="{$text.theme_button_close}">
							<a id='extension_title' href='{$extension_app_path}'>{$text.theme_title_extensions} <span id='extension_count' style='font-size: 80%;'></span></a>
							<br><br>
							<input type='text' id='extension_search' class='formfld' style='margin-left: 0; min-width: 100%; width: 100%;' placeholder="{$text.theme_label_search}" onkeyup="search_extensions('extension_list');">
						</div>
						<div id='extension_list'></div>
					</div>
				</div>
